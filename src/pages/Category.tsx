...
import { Category as Cat } from '@/types/auth';
...
const Category = () => {
	...
	const category: Cat | null = useParams().cat?.replace('_', ' ') as Cat;
	...
	useEffect(() => {
		...
		const fetchProducts = async () => {
			try {
				const res = await productService.getProducts({ category }, controller);
	...
	document.title = `${category} | ${APP_NAME}`;
	return (
			...
			<div className="mt-1 mb-3 flex w-full items-center justify-between pl-3">
				<div>
					...
					<p className="text-slate-500">
						Overview of the {category.toLocaleUpperCase()}.
					</p>
				</div>
			</div>
